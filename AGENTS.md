...
为 AMD 和 NVIDIA，Python 通过 `ctypes` 直接加载系统
`atiadlxx.dll`/`atiadlxy.dll` 或 `nvapi64.dll`，实现：

- AMD/NVIDIA GPU 与 DP/eDP 端口枚举
- DPCD 读写
- I2C-over-AUX 读写
- 多 GPU、多 DP/eDP 端口的独立寻址

Intel 当前仅有逆向研究记录，尚未形成可运行 backend。
...
- AMD 运行路径必须直接调用系统 ADL，NVIDIA 运行路径必须直接调用系统 NVAPI；
  两者都不复制或包装参考 DLL 的导出函数。
- Intel 后续实现也应由 Python 直接加载其 GPU API，不以参考 DLL 作为运行依赖。
- 公共 API 位于 `gpu_aux` 包；底层 `ctypes` 结构和私有绑定集中在 backend 模块内。
- ADL 和 NVAPI 是进程级接口，各 backend 的所有交易必须串行化，并保证初始化与
  销毁成对执行。
...
- DPCD 地址范围为 `0x00000..0xFFFFF`，AMD/NVIDIA Native AUX 单次最多 16 字节。
...
- 默认只运行只读测试，不自动运行 `tests/hardware_aux_test_amd.py` 或
  `tests/hardware_aux_test_nvidia.py`。
