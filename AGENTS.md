...
为 AMD，Python 通过 `ctypes` 直接加载系统 `atiadlxx.dll`/`atiadlxy.dll`，实现：

- AMD GPU 与 DP/eDP 端口枚举
- DPCD 读写
- I2C-over-AUX 读写
- 多 GPU、多 DP/eDP 端口的独立寻址

Intel 和 NVIDIA 当前仅有逆向研究记录，尚未形成可运行 backend。

## 架构约束

- 保持纯 Python 实现，不新增 C/C++、Cython、Rust 或自建 DLL 中间层。
- 不链接、加载或调用 `OperateCardLib.dll`。它只能在用户明确要求时作为静态逆向样本。
- AMD 运行路径必须直接调用系统 ADL，不复制或包装参考 DLL 的导出函数。
...
- DPCD 地址范围为 `0x00000..0xFFFFF`，AMD Native AUX 单次最多 16 字节。
...
- 默认只运行只读测试，不自动运行 `tests/hardware_aux_test.py`。
...
& $python -m compileall amd_aux tests
...
```powershell
& $python .\tests\hardware_aux_test.py
```
